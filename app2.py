...
async def run():
    ...
    prompt = st.text_area("Enter your prompt here", placeholder="Add your Query", label_visibility="hidden")
    is_web_search = st.toggle("Enable Web search", value=False)
    go = st.button("Go")
    ...
    if prompt and go:
        ...
        st.write_stream(call_llm(context=context, prompt=prompt, with_context=is_web_search))
